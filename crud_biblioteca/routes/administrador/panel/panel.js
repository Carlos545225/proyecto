...
router.get('/', isAuthenticated, isBibliotecarioOrAdmin, async (req, res) => {
    try {
        ...
        const totalUsuarios = await new Promise((resolve, reject) => {
            db.query(`
                SELECT 
                    COALESCE((SELECT COUNT(*) FROM lectores WHERE estado = true), 0) +
                    COALESCE((SELECT COUNT(*) FROM bibliotecarios WHERE estado = true), 0) +
                    COALESCE((SELECT COUNT(*) FROM administradores WHERE estado = true), 0) as total
            `, (err, results) => {
                if (err) {
        ...
        const librosRecientes = await new Promise((resolve, reject) => {
            db.query(`
                SELECT l.*, a.nombre_completo as autor_nombre, c.nombre as categoria_nombre
                FROM libros l
