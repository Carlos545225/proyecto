...
def init_auth_routes(app, db):
    ...
    def exportar_historias_pdf():
        ...
        try:
            ...
            if medicos_distribution:
                ...
                elements.append(Paragraph("No hay datos suficientes para generar el gráfico de distribución por médico.", normal_style))
                elements.append(Spacer(1, 20))
            
            # Estadísticas resumidas
            stats_data = [
                ['Total Historias:', str(total_historias)],
                ['Médico más activo:', max(medicos_distribution.items(), key=lambda x: x[1])[0] if medicos_distribution else 'No hay médicos'],
    ...
    def exportar_citas_pdf():
        ...
        if estados_distribution:
            ...
            elements.append(Paragraph("No hay datos suficientes para generar el gráfico de distribución por estado.", normal_style))
            elements.append(Spacer(1, 20))
        
        # Estadísticas resumidas
        stats_data = [
            ['Total Citas:', str(total_citas)],
            ['Estado más común:', max(estados_distribution.items(), key=lambda x: x[1])[0] if estados_distribution else 'No hay estados'],
            ['Médico más solicitado:', max(medicos_distribution.items(), key=lambda x: x[1])[0] if medicos_distribution else 'No hay médicos']
        ]
    ...
    def exportar_medicos_pdf():
        ...
        try:
            ...
            if genero:
                query = query.filter(Medico.genero == genero)
            ...
            if genero:
                genero_texto = 'Masculino' if genero == 'M' else 'Femenino' if genero == 'F' else 'Otro'
                filtros_texto.append(f"Género: {genero_texto}")
            ...
            especialidades_distribution = {}
            genero_distribution = {}
            
            for m in medicos:
                especialidades_distribution[m.especialidad] = especialidades_distribution.get(m.especialidad, 0) + 1
                genero_distribution[m.genero] = genero_distribution.get(m.genero, 0) + 1
            
            # Crear gráfico de especialidades solo si hay datos
            if especialidades_distribution:
                plt.figure(figsize=(8, 4))
                plt.pie(especialidades_distribution.values(), 
                       labels=[ESPECIALIDADES.get(esp, esp) for esp in especialidades_distribution.keys()], 
                       autopct='%1.1f%%', colors=plt.cm.Pastel1.colors)
                plt.title('Distribución por Especialidad', pad=20)
                plt.axis('equal')
                ...
                elements.append(Paragraph("No hay datos suficientes para generar el gráfico de distribución por especialidad.", normal_style))
                elements.append(Spacer(1, 20))
            
            if not medicos:
                elements.append(Paragraph("No hay médicos registrados que coincidan con los criterios de búsqueda.", normal_style))
                elements.append(Spacer(1, 20))
            else:
                # Tabla de médicos
            ...
            stats_data = [
                ...
                ['Especialidad más común:', ESPECIALIDADES.get(max(especialidades_distribution.items(), key=lambda x: x[1])[0], 'No especificada') if especialidades_distribution else 'No hay datos'],
                ['Género predominante:', 'Masculino' if max(genero_distribution.items(), key=lambda x: x[1])[0] == 'M' else 'Femenino' if max(genero_distribution.items(), key=lambda x: x[1])[0] == 'F' else 'Otro' if genero_distribution else 'No hay datos']
            ]
    ...
    def exportar_pacientes_pdf():
        ...
        try:
            ...
            # Crear gráfico de EPS solo si hay datos
            if eps_distribution:
                plt.figure(figsize=(8, 4))
                ...
                elements.append(Paragraph("No hay datos suficientes para generar el gráfico de distribución por EPS.", normal_style))
                elements.append(Spacer(1, 20))
            
            if not pacientes:
                elements.append(Paragraph("No hay pacientes registrados que coincidan con los criterios de búsqueda.", normal_style))
                elements.append(Spacer(1, 20))
            else:
            ...
            stats_data = [
                ...
                ['EPS más común:', max(eps_distribution.items(), key=lambda x: x[1])[0] if eps_distribution else 'No hay datos'],
                ['Ciudad más común:', max(ciudades_distribution.items(), key=lambda x: x[1])[0] if ciudades_distribution else 'No hay datos']
            ]
