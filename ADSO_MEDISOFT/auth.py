...
from model import db, usuario, medico as Medico, paciente as Paciente, cita, historia_clinica, factura as Factura
...
def init_auth_routes(app, db):
    ...
    def agregar_historia():
        ...
        try:
            ...
            nueva_historia = historia_clinica(
                ...
                diagnostico=request.form.get('diagnosticoActual'),
    ...
    def editar_historia(id):
        ...
        try:
            ...
            historia_actual.motivo_consulta = request.form.get('editMotivoConsulta')
            historia_actual.antesedentes = request.form.get('editAntecedentesMedicos')
            historia_actual.diagnostico = request.form.get('editDiagnosticoActual')
            historia_actual.tratamiento = request.form.get('editTratamientoActual')
    ...
    def exportar_medicos_pdf():
        ...
        direcciones_distribution = {}
        
        for m in medicos:
            especialidad_nombre = ESPECIALIDADES.get(m.especialidad, 'Especialidad no encontrada')
            especialidades_distribution[especialidad_nombre] = especialidades_distribution.get(especialidad_nombre, 0) + 1
            direcciones_distribution[m.direccion] = direcciones_distribution.get(m.direccion, 0) + 1
        
        # Crear gráfico de especialidades
        plt.figure(figsize=(8, 4))
        plt.pie(especialidades_distribution.values(), labels=especialidades_distribution.keys(), 
                autopct='%1.1f%%', colors=plt.cm.Pastel1.colors)
        ...
        stats_data = [
            ...
            ['Especialidad más común:', max(especialidades_distribution.items(), key=lambda x: x[1])[0]],
            ['Dirección más común:', max(direcciones_distribution.items(), key=lambda x: x[1])[0]]
        ]
        ...
        # Encabezado para la tabla de médicos
        elements.extend(create_report_header(doc, "Listado de Médicos", "Detalle completo de médicos registrados"))
        
        # Tabla de médicos
        data = [['ID', 'Nombre', 'Apellido', 'Especialidad', 'Documento', 'Teléfono', 'Correo', 'Dirección']]
        for m in medicos:
            especialidad_nombre = ESPECIALIDADES.get(m.especialidad, 'Especialidad no encontrada')
            data.append([
        ...
        # Ajustar anchos y alinear a la izquierda correo y dirección
        col_widths = [40, 70, 70, 90, 70, 70, 150, 150]
    ...
    # Función para exportar pacientes a PDF
    ...
    def exportar_pacientes_pdf():
        ...
        estado_civil_distribution = {}
        
        for p in pacientes:
            eps_distribution[p.eps] = eps_distribution.get(p.eps, 0) + 1
            ciudades_distribution[p.ciudad] = ciudades_distribution.get(p.ciudad, 0) + 1
            estado_civil_distribution[p.estado_civil] = estado_civil_distribution.get(p.estado_civil, 0) + 1
        ...
        stats_data = [
            ...
            ['EPS más común:', max(eps_distribution.items(), key=lambda x: x[1])[0]],
            ['Ciudad más común:', max(ciudades_distribution.items(), key=lambda x: x[1])[0]],
            ['Estado civil más común:', max(estado_civil_distribution.items(), key=lambda x: x[1])[0]]
        ]
        
        stats_table = create_modern_table(stats_data, [200, 200], header_bg=accent_color)
        elements.append(stats_table)
        elements.append(PageBreak())
        elements.extend(create_report_header(doc, "Listado de Pacientes", "Detalle completo de pacientes registrados"))
